...
function editProfile(data) {
  ...
  if (!warning) {
    ...
    warning.style.backgroundColor = '#D3AD83';
...
function submitForm() {
  ...
  if (currentStep === 1 || isEditing) {
    ...
    if (username.length > 15) {
      showError('username', 'Username deve ter no máximo 15 caracteres.');
      valid = false;
    }
  }
  if (!isEditing) {
...
document.getElementById('proceedDelete').addEventListener('click', () => {
  const cpfInput = prompt('Informe seu CPF para confirmar a exclusão:');
  if (cpfInput) {
